...
class AgendaStatus extends React.Component {
  componentDidMount() {
    this._adjustHeader()
  }
...
AgendaStatus.range = (start, { length = AgendaStatus.defaultProps.length }) => {
  let end = dates.add(start, length, 'day')
  return { start, end }
}

AgendaStatus.navigate = (
  date,
  action,
  { length = AgendaStatus.defaultProps.length }
) => {
  switch (action) {
...
AgendaStatus.title = (
  start,
  { length = AgendaStatus.defaultProps.length, localizer }
) => {
  let end = dates.add(start, length, 'day')
